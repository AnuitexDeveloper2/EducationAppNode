...
const LastColumn =({value,assigment}) => {
  ...
  const showEdit = () => {
    ...
    if (assigment==="author") {
      toggle("createAuthor")
      setState({isShowCreateAuthor: true, isShowRemove: false, isShowCreateProduct: false})
    }
    if(assigment==="product") {
      toggle("createProduct");
      setState({isShowCreateAuthor: false, isShowRemove: false, isShowCreateProduct: true})
    }
  }
  const showRemove = () => {
    setState({isShowRemove:true,isShowCreateAuthor:false, isShowCreateProduct:false})
    toggle("remove")
  }
    return(
<div>
