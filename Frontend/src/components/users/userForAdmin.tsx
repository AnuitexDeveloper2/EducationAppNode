...
const UsersForAdmin = () => {
  ...
  const getData = async (pages, userType) => {
    ...
    setData({
      users: users.data,
      pages: Math.floor(users.count / 10 + 1),
      userType: UserFilterType.All,
    });
  };
    ...
    },
    {
      Header: (props) => {
        return (
          <div>
            <span>
              Status <img src={Poligon} alt="" onClick={filterStatus} />
            </span>
            <div>{modal.showStatus && <PickStatus setData={setData} />}</div>
          </div>
        );
      },
      id: "status",
  ...
  return (
    <div>
      <div className="author-header">
        <div className="authors-title">Users Managment</div>
        <br />
      </div>
      <ReactTable
            ...
            onClick: () => {
              if (rowInfo !== undefined) {
                const currentUser = {
                  ...
                  name: rowInfo.original.userName,
                };
  ...
  const selectStatus = async (e) => {
    ...
    const type =
      e.target.value === "Active"
        ? (idx = UserFilterType.Blocked)
        : (idx = UserFilterType.Active);
    const filter: UserFilter = {
      searchString: "",
      pageNumber: 1,
      pageSize: 10,
